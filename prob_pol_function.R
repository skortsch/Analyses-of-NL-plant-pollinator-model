...
#---------DEFINING SOME VARIABLES---------------------------
  ...
  v_n<-nrow(ex_run)                               #tot_number of visits
  ...
  #dataset
  vis_data<-data.frame('f_id'=ex_run$plant_patch,'f_sp'=ex_run$plant_species,'p_id'=ex_run$pollinator_agent)
...
#---------Pollination probability---------------------------  
  ...
  for (p_id_ in 1:pol_n){
    ...
    visits<-vis_data[vis_data$p_id==ags.id[p_id_],]
    v_n_<-nrow(visits)
    if (v_n_>1){
    for (i in 1:(v_n_-1)){
      f_sp_<-visits[i,2]
...
res<-list(run, number_visits_sum, number_visits_mean, number_vis_per_plant_sum, number_vis_per_plant_mean) #seed_percent, 
names(res)<- c("run", "number_visits_sum", "number_visits_mean", "number_vis_per_plant_sum", "number_vis_per_plant_mean") #"seed_percent",
return(res)
}
