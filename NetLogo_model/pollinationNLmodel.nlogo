...
to setup-pollinators
  ...
  if not file-exists? (word "pol_pars_" replicate_n ".csv") [
    user-message (word "pol_pars_" replicate_n ".csv")
    stop
  ]
  ...
  while [ not file-at-end? ] [
    ...
    create-pollinators 1 [
      ...
      set perception_angle item 10 pollinator_data
                                                            ; Should add memory_extinction 1/600 = 1 Day
...
to body-mass-dependent-distance
  if body_mass > 0 [                                  ; Parametrize flight_speed and max_distance using Liam's model
    (ifelse eusocial = 3                                ; highly eusocial
      [
        set flight_speed ( exp (5.34 + body_mass * 0.3) * 10) / 600
        set max_distance   exp (5.34 + body_mass * 0.3)
      ]
      eusocial = 2                                      ; primitively eusocial
      [
        set flight_speed ( exp (5.34 - 1.12 + body_mass *  0.3 ) * 10 ) / 600
        set max_distance ( exp (5.34 - 1.12 + body_mass *  0.3 ) )
      ]
      eusocial = 1                                      ; solitary with nest
      [
        set flight_speed ( exp (5.34 - 1.13 + body_mass * 0.3 ) * 10 ) / 600
...
to setup-plants
  ...
  if not file-exists? (word "plant_pars_" replicate_n ".csv") [
    user-message "No file 'plant_pars.csv' exists!"
    stop
  ]
...
to go
  ...
  if not any? pollinators or day = number-of-days [               ; 600 ticks per day
    file-close
