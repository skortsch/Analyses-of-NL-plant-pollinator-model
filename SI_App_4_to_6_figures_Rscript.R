...
bs1 <- mean.vis.plant%>% mutate(cut_conn = cut(connectance,breaks=c(0.24, 0.35, 0.45, 0.55, 0.65, 0.75, 0.85, 0.95, 1))) %>% 
  ggplot( aes(x = factor(bins_seed), y = number_visits)) + xlab("Plant intermixing [log]") + 
...
bs2 <- mean.vis.plant%>% mutate(cut_conn = cut(connectance,breaks=c(0.24, 0.35, 0.45, 0.55, 0.65, 0.75, 0.85, 0.95, 1))) %>% 
  ggplot( aes(x = factor(bins_seed), y = cons)) + xlab("plant intermixing [log]") + 
...
bs3 <-mean.vis.plant%>% mutate(cut_conn = cut(connectance,breaks=c(0.24, 0.35, 0.45, 0.55, 0.65, 0.75, 0.85, 0.95, 1))) %>% 
  ggplot( aes(x = factor(bins_seed), y = pvis)) + xlab("plant intermixing [log]") + 
...
#Appendix 6: figures 1-3
#visits per plant species

vis.per.plant$seed_logged<-log(vis.per.plant$seed_percent)
...
tags <- c( "[(-12)-(-9))", "[(-9)-(-6))", "[(-6)-(-3))", 
           "[(-3)-0)")
# bucketing values into bins
vis.per.plant$bins_seed <- cut(vis.per.plant$seed_logged, breaks=breaks, include.lowest=TRUE, right=FALSE, labels=tags)

#plant densities
breaks1 <- c( 0.1, 0.3, 0.5, 0.7, 0.9)

# specify interval/bin labels
tags1 <- c( "[0.1-0.3)", "[0.3-0.5)", "[0.5-0.7)", "[0.7-0.9)")

vis.per.plant$bins_dens<-cut(vis.per.plant$pl.dens, breaks=breaks1, include.lowest=TRUE, right=FALSE, labels=tags1)

#Vistation rate
#mean.vis.plant %>% group_by(bins_seed) %>% summarise()
ts<-vis.per.plant%>% mutate(cut_pol_links = cut(pol.links ,breaks=c(0,2,4,6))) %>% 
  group_by(bins_seed, bins_dens, cut_pol_links) %>% summarise(mean_vis=mean(number_visits), sd_vis=sd(number_visits), 
  n = n(), se = sd_vis / sqrt(n), ymin=mean_vis-se, ymax=mean_vis+se)


ts %>%  ggplot(aes(x = factor(bins_seed), y = mean_vis, ymin=ymin, ymax=ymax))+ 
  theme_light()+ facet_grid(cols=vars(cut_pol_links), rows=vars( bins_dens),labeller ="label_both", scales = "free") + 
  guides(x =guide_axis(angle = 90)) + 
  theme(strip.text.x = element_text(size = 9, color = "black"))+
  theme(strip.text.y = element_text(size = 9, color = "black"))+
  theme(strip.background = element_rect(color="grey90", fill="grey90", size=1.5, linetype="solid"))+
  theme(axis.title.y = element_text(margin = margin(r = 10)))+
  geom_linerange()+
  geom_pointrange()+
  geom_point(col="red", size=2) +
  xlab("plant intermixing [log]")+
  ylab("visitation rate")

ggsave(paste0(dirF, "SI_App7_Fig1_vis.png"),width=8, height = 10, units="in", dpi=600 ) 

#Consecutive visits
ts2<-vis.per.plant%>% mutate(cut_pol_links = cut(pol.links ,breaks=c(0,2,4,6))) %>% 
  group_by(bins_seed, bins_dens, cut_pol_links) %>% summarise(mean_vis=mean(cons), sd_vis=sd(cons), 
  n = n(), se = sd_vis / sqrt(n), ymin=mean_vis-se, ymax=mean_vis+se)


ts2 %>%  ggplot(aes(x = factor(bins_seed), y = mean_vis, ymin=ymin, ymax=ymax))+ 
  theme_light()+ facet_grid(cols=vars(cut_pol_links), rows=vars( bins_dens),labeller ="label_both", scales = "free") + 
  guides(x =guide_axis(angle = 90)) + 
  theme(strip.text.x = element_text(size = 9, color = "black"))+
  theme(strip.text.y = element_text(size = 9, color = "black"))+
  ...
  geom_point(col="red", size=2)+ 
  xlab("plant intermixing [log]")+
  ylab("consecutive visits")

ggsave(paste0(dirF, "SI_App7_Fig2_cons.png"),width=8, height = 10, units="in", dpi=600 ) 

#Expected number of plants pollinated
ts3<-vis.per.plant%>% mutate(cut_pol_links = cut(pol.links ,breaks=c(0,2,4,6))) %>% 
  group_by(bins_seed, bins_dens, cut_pol_links) %>% summarise(mean_vis=mean(pvis), sd_vis=sd(pvis), 
  n = n(), se = sd_vis/sqrt(n), ymin=mean_vis-se, ymax=mean_vis+se)

ts3 %>%  ggplot(aes(x = factor(bins_seed), y = mean_vis, ymin=ymin, ymax=ymax))+ 
  theme_light()+ facet_grid(cols=vars(cut_pol_links), rows=vars( bins_dens),labeller ="label_both", scales = "free") + 
  guides(x =guide_axis(angle = 90)) + 
  theme(strip.text.x = element_text(size = 9, color = "black"))+
  theme(strip.text.y = element_text(size = 9, color = "black"))+
  ...
  geom_point(col="red", size=2)+ 
  xlab("plant intermixing [log]")+
  ylab("expected number of plants pollinated")

ggsave(paste0(dirF, "SI_App7_Fig3_pvis.png"),width=8, height = 10, units="in", dpi=600 ) 
